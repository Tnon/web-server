  ...
  <div class="dcontent">
    <div class="header">
      <span class="title" @click="goBack">
        ...
        <customIcon class="cusicon" v-if="view === 'Ip'" :type="logoType" size="24" />
      ...
      <div class="rightOpt">
        ...
        <a-input-search
          ...
          :placeholder="t('asset.pInput', [t(`asset.vultyNameOrAccount`)])"
          @search="searchData"
          @change="searchData"
        />
        <!-- 主机视图搜索 end -->
    ...
    <slots-table
      ref="tableRef"
      :scroll="{ x: '500px', y: 'calc(100vh - 223px)' }"
      class="tabCon hasSubTable"
      v-model:columns="columns"
      :row-key="view === 'Pro' ? 'host_id' : 'vul_id'"
      :get-list-func="api"
      :arguments="agms"
    >
      <template #bodyCellcustom="{ record, column }">
        <template v-if="column.dataIndex === 'detail'">
          <span @click="showDrawer(record)" class="primary_color">{{ t('asset.lookOver') }}</span>
        </template>
      </template>
    </slots-table>
    <!-- 表格 end -->
...
<script lang="ts" setup>
  ...
  const { vul_id, title, host_id, type, view, logoType } = currentRoute.query;
  // 漏洞页资产详情和ip详情的表格字段都一样，所以不区分何种漏洞,只区分Pro和Ip
  // 如后期不同中漏洞字段不一样，可参照资产信息中进行取值
  const columns: TableColumn[] = getapi_columnsDetail('' + view, t, currentLang.value);
  ...
  //table Arguments
  // 资产详细视图参数：name: vul_id, searchdata：主机ip
  // IP详细视图参数：host_id，searchdata， vul_category
  const agms = reactive<Arguments>({
    [view === 'Ip' ? 'host_id' : 'name']: view === 'Ip' ? (host_id as string) : Number(vul_id),
    searchdata: '',
    vul_category: vulType[type as string],
  });
  //go Pro or Ip
  const goBack = () => {
    router.go(-1);
    ...
    useCreatDraw(
      {
        ...
        contentProps: {
          record,
          host_id: view === 'Ip' ? host_id : record.host_id,
        },
      },
      draw,
    );
