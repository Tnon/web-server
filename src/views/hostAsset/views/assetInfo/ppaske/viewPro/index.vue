...
<script lang="ts" setup>
  ...
  watch(
    ...
    (type) => {
      if (!type) return;
      if (unref(currentRoute.name as string).indexOf('assetInfo') > -1) {
  ...
  //switchView
  const switchView = () => {
    router.push({ name: 'hostAsset-layout-assetInfo-' + state.type + '-viewIp' });
  };

  //go to detail page
  const jumpDetail = (record) => {
    router.push({
      name: 'hostAsset-layout-assetInfo-' + state.type + '-detail',
      query: {
        title: record.name,
        type: state.type,
        view: 'Pro',
