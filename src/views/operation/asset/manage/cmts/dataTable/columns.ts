 ...
 * @LastEditTime: 2023-04-07 17:57:50
 */
    ...
    actions: [
      {
        ...
        actionList: [
              ...
              useModal(
                {
                  ...
                  handleOk: async (conRef) => {
                    ...
                    await modifyHostInfo(Object.assign({}, conRef.fields, { mac: record.host_id }));
                    callback();
                  },
                  contentProps: {
                    fields: reactive({
          ...
          {
            text: T('asset.moveGroupTitle'),
            func: ({ record }, callback) => {
              useModal(
                {
                  title: T('asset.moveHostG'),
                  handleOk: async (conRef) => {
                    await conRef.validate();
                    const sendObj = {
                      group_id: conRef.group_idState.group_id,
                      host_id_array: record.host_id,
                    };
                    await move_Group(sendObj);
                    callback();
                  },
                  contentProps: {
                    group_idState: reactive({
                      group_id: undefined,
                    }),
                  },
                },
