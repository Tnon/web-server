 ...
 * @LastEditTime: 2023-10-26 20:09:26
 */
...
import { delWbPolicy, editBlist, getWbListDetai } from '@/api/strategy/white';
          ...
          actionList: [
            {
              ...
              func: async ({ record }, callback) => {
                const { id, remarks, rules_detail } = await getWbListDetai({
                  id: record.id!,
                  policy_type: E_WB_Enum.Black,
                });
                useModal(
                  {
                    ...
                    contentProps: {
                      ...
                      record: {},
                      modelType: 0,
                      formStateRes: { id, remarks, rules_detail },
                    },
                    handleOk: async (contRef) => {
                      await contRef.validate();
                      await editBlist(contRef.formState);
                      callback();
                    },
                ...
                useModal(
                  {
                    ...
                    handleOk: async () => {
                      await delWbPolicy({ id_list: record.id + '', policy_type: E_WB_Enum.Black });
                      callback!();
                    },
                  },
