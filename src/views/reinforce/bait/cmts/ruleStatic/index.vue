 ...
 * @LastEditTime: 2023-07-24 14:31:37
-->
<template>
...
<script setup lang="ts">
  ...
  import { getRuleStaticDf } from '@/api/reinforce';
  const { t } = useI18n();
  ...
  const getStaticCount = async () => {
    const {
      ...
      // //受保护文件
      // protected_file = 0,
    } = await getRuleStaticDf();
    modelView.monitored_bait_rule.count = custom_rule + preset_rule;
    modelView.monitored_bait_rule.extra![0].count = custom_rule;
    modelView.monitored_bait_rule.extra![1].count = preset_rule;
