 ...
 * @LastEditTime: 2023-10-13 10:09:40
-->
<template>
  <div class="con">
    <div class="node">
      <div
        class="leftOp"
        :class="{ canJump: type === 'monitored_rule' || type === 'monitored_host' }"
      ...
      <template v-if="item.extra">
        <div class="line"></div>
        <template v-for="(extra, index) in item.extra" :key="extra.title">
          <!-- 标题 start -->
          <div
            class="extramsg"
            @click.stop="viewDetail(type, !index ? E_Rule_Type.Custom : E_Rule_Type.Preset)"
          >
...
<script setup lang="ts">
  ...
  //点击查看
  const viewDetail = (type: string, rule_type?) => {
    if (type === 'monitored_bait_rule') {
      router.push({
    ...
    } else if (type === 'monitored_host') {
      router.push({
        name: 'reinforce-layout-bait-host',
        query: {
          rule_type,
        },
      });
