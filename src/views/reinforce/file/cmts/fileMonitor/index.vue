 ...
 * @LastEditTime: 2023-03-24 10:59:46
-->
<template>
  <div class="content">
    <!-- 内容头部  start-->
    <div class="header">
      ...
      <div class="roption">
        <div class="tools">
          ...
          <!-- 分割线 end -->
          <!-- 导出 start -->
          <export-data api="/api/report/fmrulestatuslist" :params="state" />
          <!-- 导出 end -->
        </div>
      </div>
    </div>
    ...
    <slots-table
      ...
      row-key="rule_id"
      :get-list-func="getRuleHostDetail"
      :arguments="state"
    />
    <!-- 表格 end -->
  </div>
...
<script lang="ts" setup>
  ...
  import { getRuleHostDetail } from '@/api/reinforce';
  ...
  //条件的结构体,导出参数
  const state = reactive<I_File_Monitor_Filter & { host_id: string }>({
    rule_name: '',
    //文件路径
    file_path: '',
    //监控模式
