 ...
 * @LastEditTime: 2023-08-02 18:15:11
-->
<template>
    ...
    <a-layout-sider :trigger="null" class="slider" theme="light">
      ...
      <div class="header">
        ...
        <div class="iconContainer">
          <custom-icon class="icon" type="#ax-Protection" />
        </div>
        <span>{{ t('menu.reinforce') }}</span>
      </div>
      <!-- 标题 end -->
      <div class="line"></div>
      <!--      侧边菜单栏start-->
      <aside-menu class="asidemenuop" :menus="allRouters" routerName="reinforce" />
      <!--      侧边菜单栏end-->
    </a-layout-sider>
    <!--      内容区域start-->
...
<script setup lang="ts">
  import reinforceRouters from '@/router/modules/reinforce/modules';
  ...
  watchEffect(() => {
    if (unref(acdr_enabled)) {
      allRouters.value = reinforceRouters;
    } else {
      allRouters.value = deleteNode(cloneDeep(reinforceRouters), 'reinforce-layout-file');
    }
  });
</script>
